/* vendor/alisilia-auth.js */
(function (global) {
  'use strict';

  var FUNCTIONS_BASE = 'https://evrulqmiiswsidgsysiq.supabase.co/functions/v1';

  var SESSION_KEY = 'alisilia_session';
  var PENDING_SDIFK_KEY = 'alisilia_pending_sdifk';
  ...
  function injectHeaderStyles() {
    ...
    style.textContent =
      ...
      '.avatar-btn{width:40px;height:40px;border-radius:50%;border:1px solid var(--line,#e4e4e0);' +
      'padding:0;background:none;cursor:pointer;display:flex;align-items:center;justify-content:center;' +
      'overflow:hidden;transition:border-color .15s ease;}' +
      '.avatar-btn:hover{border-color:var(--ink,#0a0a0a);}' +
      '.avatar-btn img{width:100%;height:100%;object-fit:cover;display:block;}' +
      '.user-dropdown{position:absolute;right:0;top:calc(100% + 8px);background:var(--paper,#fff);' +
      'border:1px solid var(--line,#e4e4e0);border-radius:12px;padding:6px;min-width:180px;' +
      'box-shadow:0 10px 30px rgba(0,0,0,.12);display:none;z-index:1000;}' +
      '.user-dropdown.open{display:block;}' +
      '.dropdown-item{display:flex;align-items:center;gap:10px;width:100%;padding:10px 12px;' +
      'border-radius:8px;font-size:14px;cursor:pointer;color:var(--ink,#0a0a0a);background:none;' +
      'border:none;font-family:inherit;transition:background .12s ease;}' +
      '.dropdown-item:hover{background:var(--paper-dim,#f6f6f4);}' +
  ...
  function renderHeader(authAreaEl) {
    ...
    if (!session) {
      authAreaEl.innerHTML =
        '<a href="/enter" class="btn btn-solid" style="color:#ffffff; text-decoration:none;">Enter</a>';
      return;
    }
    ...
    return new Promise(function (resolve) {
      ...
      if (downloadBtn) {
        downloadBtn.addEventListener('click', function () {
          var blob = new Blob(
            ['Your Flowery Key for Alisilia (@' + session.username + '):\n\n' + pendingSdifk +
             '\n\nThis is the ONLY copy. Alisilia does not store it and cannot show it to you again.\n'],
            { type: 'text/plain' }
          );
